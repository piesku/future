...
export const modern_ground = (game: Game, x: number, y: number): Blueprint => ({
    Translation: [x, 0, y],
    Rotation: from_euler([0, 0, 0, 1], 0, element([0, 90, 180]), 0),
    Using: [render_textured(game.MaterialTextured, game.MeshCube, game.Textures.road)],
});

export let modern_structure = (
    game: Game,
    start_time: number,
    end_time: number
): Array<Blueprint> => {
    ...
    for (let i = 0; i < segments; i++) {
        Children.push({
            Translation: [0, -1, 0],
            Using: [
                render_textured(game.MaterialTextured, game.MeshCube, game.Textures.skyscraper),
                time_control([
                    {
